...
export function CareerCard({ career, onOpenChange }: {career: Carrera, onOpenChange: (open: boolean) => void }) {
    ...
    return (
        <div
        key={career.id}
        className="flex items-center justify-between p-4 rounded-none rounded-lg border border-border hover:border-foreground/20 hover:bg-muted/30 transition-all group"
        ...
        >
            ...
            <Button
                size="sm"
                variant="ghost"
                className="group-hover:bg-foreground cursor-pointer rounded-none group-hover:text-background"
            >
